...
class Expression {
    ...
    static createUserType(position, args) {
...
class AnnotatedExpression extends Expression {
    constructor(position, annotations) {
        super(position);
...
class UserTypeExpression extends Expression {
...
function createExpression(type, position, ...args) {
    switch (type) {
        ...
        case tokens_1.Token.UserType: return Expression.createUserType(position, args);
